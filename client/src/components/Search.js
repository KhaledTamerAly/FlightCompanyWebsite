  ...
  render() {
    ...
    return (
      ...
      <button onClick={this.userInput.bind(this)}>Search</button>  
          ...
          <ul>
            {
              (this.state.flightToBeListed ?? []).map(option => <li><CardPanel title={option.flightNumber} subtitle="" content={"From:  "+ option.departureTerminal+ " "+ "To: "+option.arrivalTerminal + " " +"On: "+ option.flightDate} /></li>)
            }
          </ul>
    </div>
    );
